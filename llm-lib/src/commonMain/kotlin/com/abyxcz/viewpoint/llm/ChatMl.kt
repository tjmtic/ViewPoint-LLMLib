...
/**
 * One-turn prompts in the ChatML family (MiniCPM5, Qwen, …), rendered the way the model's own Jinja
 * chat template renders them. llama.cpp's C API cannot run those templates and the shim passes
 * prompts through untouched, so the format lives here.
 *
...
class ChatMl(val bos: String, val thinkingSwitch: Boolean = true) {
     ...
     */
    fun prompt(user: String, system: String? = null, thinking: Boolean = false): String =
        buildString {
            append(bos)
